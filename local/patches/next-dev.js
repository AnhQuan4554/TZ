...
let _indexJs = _interopRequireDefault(
  require('next/dist/compiled/arg/index.js'),
);
let _fs = require('fs');
let _startServer = require('../server/lib/start-server');
let _utils = require('../server/lib/utils');
let Log = _interopRequireWildcard(require('../build/output/log'));
let _output = require('../build/output');
let _isError = _interopRequireDefault(require('../lib/is-error'));
let _getProjectDir = require('../lib/get-project-dir');
let _constants = require('../shared/lib/constants');
...
const nextDev = (argv) => {
  ...
  if (!(0, _fs).existsSync(dir)) {
    (0, _utils).printAndExit(
      `> No such directory exists as the project root: ${dir}`,
    );
  }
  ...
  (0, _startServer)
    ...
    .then(async (app) => {
      ...
      (0, _output).startedDevelopmentServer(
        appUrl,
  ...
  for (const CONFIG_FILE of _constants.CONFIG_FILES) {
    (0, _fs).watchFile(
      _path.default.join(dir, CONFIG_FILE),
      { interval: 99999999 },
      (cur, prev) => {
        // console.log(cur, prev, CONFIG_FILE, dir);
  ...
  console.log('Something went wrong, next dev exited');
};
