 ...
 * @returns
 */
export async function webProxy<T = any>(
  url: string,
  ...
  console.log({ url, method, query }, `Proxy request to -- > ${url}`);
  try {
 ...
 */
export async function getProxy<T = any>(
  url: string,
...
export async function postProxy<T = any>(
  url: string,
  authorizationHeader: string,
  payload: any,
) {
