...
const RootPublishButton = ({ isPublished, onPublishedSuccess }: props) => {
  ...
  const onPublish = async () => {
    setLoading(true);
    try {
      const queryResult = await publish('root-authority/publish');
      if (queryResult.data.success) {
        setLoading(false);
        toast.success('Root published successfully.');
        setIsDisabled(true);
        ...
        toast.error(
          `Failed to publish root authority.  ${queryResult.data.message}`,
        );
      }
    } catch (err: any) {
      setLoading(false);
      onPublishedSuccess(false);
      toast.error(`Failed to publish root authority.  ${err.message}`);
    }
  };
  ...
  return (
    <>
      ...
      <Dialog open={showModal} aria-labelledby="confirm-dialog">
        ...
        <DialogContent>
          Are you sure you want to publish root authority data to Guardian ?
        </DialogContent>
        <DialogActions>
          <Button
            variant="contained"
...
export default RootPublishButton;
