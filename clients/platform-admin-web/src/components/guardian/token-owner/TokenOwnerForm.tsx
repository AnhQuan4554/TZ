...
import { fetchTokenOwnerData } from '../../../api/useFetchTokenOwnerData';
import { ContactForm } from '../ContactForm';
import { AddressForm } from '../AddressForm';
import { ContactFormBusinessLead } from '../ContactFormBusinessLead';
import RootPublishButton from './TokenPublishButton';
import { Loading } from '../../Loading';
import CountrySelect from '../../CountrySelect';

export function TokenOwnerForm() {
  ...
  const mutateForm = useForm<ITokenOwner>(
    'token-owner',
    isUpdating ? 'PUT' : 'POST',
  );
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      tokenOwnerName: Yup.string().required('Business Name is required'),
        ...
        { ...(formik.values as any) },
        {
          onSuccess: (res: any) => {
            if (res.data.success) {
              ...
              if (isUpdating) {
                toast.success('Token Owner updated');
              } else {
                toast.success('Token Owner added');
                setIsUpdating(true);
              }
            } else {
              helpers.setStatus({ success: false });
  ...
  const { refetch, isLoading } = useQuery(
    ['token-owner'],
    async () => {
      const data = await fetchTokenOwnerData();
      if (data) {
            ...
            <Grid container spacing={2}>
              <Grid item xs={6}>
                ...
                <TextField
                  ...
                  margin="normal"
                  autoFocus
                  value={formik.values.tokenOwnerName}
                  onChange={formik.handleChange}
                  error={Boolean(
                    formik.touched.tokenOwnerName &&
                      formik.errors.tokenOwnerName,
                  )}
                  helperText={
                    formik.touched.tokenOwnerName &&
                    formik.errors.tokenOwnerName
